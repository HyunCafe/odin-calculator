...
My final project for TOP is a fully-functional calculator built with pure HTML, CSS, and JavaScript, showcasing my skills in building complex UI components with an intuitive user experience. The calculator supports basic arithmetic operations, features hover effects for visual feedback, and a top navigation bar with minimize, maximize, and close buttons. The resizable feature works like the Windows calculator, while the improved minimize and restore feature remembers the last position and size of the calculator. The project also includes unique features such as Windows desktop simulation, search box, and a right-click menu.
